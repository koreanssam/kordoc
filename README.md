...
한컴에서 저장한 실물 HWPX를 골격으로 사용하는 내장 프리셋입니다. 원본의 글꼴·장평·
자간·행간·2단 편집·표 테두리·셀 병합을 유지하면서 문항과 채점 행을 동적으로 늘립니다.
...
```bash
kordoc generate 고사.md -o 고사원안.hwpx --preset 고사원안
```

채점기준표는 `과목·학년도·학기·차수·학년·반영점수·출제자` frontmatter와
`문항 | 정답 | 인정정답 및 채점기준 | 배점` 4열 GFM 표를 사용합니다. 셀 안 줄바꿈은
`<br>`로 표시합니다.
